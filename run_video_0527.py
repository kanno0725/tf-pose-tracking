...
# 読み込む動画のパス
movie_file = 'one_minutes.mp4'
output_file = 'crop4_1_addweld_result_test20527.mp4'
output_csv1 = "id_list_crop4_1.csv"
...
while True:
    ...
    # test-0527 img = TfPoseEstimator.draw_humans(img, humans, imgcopy=False)
    ...
    for human in humans:
        ...
        for m in human.body_parts:
            ...
            center = np.array([int(body_part.x * size[0] + 0.5), int(body_part.y * size[0] + 0.5)])
            ...
            center_sum += center
        center_ave = center_sum/parts_num 
        ...
        if parts_num >= 5:
            ...
            #ここにその人間の座標の平均を入れたい
            center_ave_l = center_ave.tolist()
            coordinate_frame.append(list(map(int,center_ave_l)))
    ...
    if frame_no == 0:
        ...
        for human_no in range(xx):
            img = cv2.putText(img,'id:'+str(id_list[human_no]),(coordinate_frame[human_no][0],coordinate_frame[human_no][1]),cv2.FONT_HERSHEY_PLAIN,1,(255, 255, 255), 1, cv2.LINE_AA)
        
        for weld_no in range(xx,len(id_list)):
            img = cv2.putText(img,'id:'+str(id_list[weld_no]),(coordinate_frame[weld_no][0],coordinate_frame[weld_no][1]),cv2.FONT_HERSHEY_PLAIN,1,(255, 0, 0), 1, cv2.LINE_AA)
            img = cv2.circle(img, (coordinate_frame[weld_no][0],coordinate_frame[weld_no][1]), 100, (255, 0, 0),3)
        ...
        if id_exist:
            ...
            if len(id_list_update) == 0:
                ...
                # 取得した人物のidをフレーム画像に描画
                for human_no in range(xx):
                    img = cv2.putText(img,'id:'+str(id_list[human_no]),(coordinate_frame[human_no][0],coordinate_frame[human_no][1]+5),cv2.FONT_HERSHEY_PLAIN,1,(255, 255, 255), 1, cv2.LINE_AA)
                
                for weld_no in range(xx,len(id_list)):
                    img = cv2.putText(img,'id:'+str(id_list[weld_no]),(coordinate_frame[weld_no][0],coordinate_frame[weld_no][1]+5),cv2.FONT_HERSHEY_PLAIN,1,(255, 0, 0), 1, cv2.LINE_AA)
                    img = cv2.circle(img, (coordinate_frame[weld_no][0],coordinate_frame[weld_no][1]), 100, (255, 0, 0),3)
    
    # 合成した画像をフレームとしてアウトプットに追加
    #cv2.imwrite(str(frame_no)+'.jpg', img)
    #del img
    vw.write(img)
